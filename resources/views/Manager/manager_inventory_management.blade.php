...
@section('page-title', 'City Burgers POS | Inventory Management')
...
@section('content')
                        ...
                        <div class="inventory-card shadow-sm" style="position: relative;">
                            <div style="position: absolute; top: 5px; right: 5px;">
                                <i class="fa-solid fa-trash text-danger delete-inventory" style="font-size: 1.2rem;"
                                    data-id="{{ $inventory->id }}"></i>
                            </div>
                            <div class="profile-picture mb-2">
                                <img src="{{ $inventory->item_image ? '/storage/' . $inventory->item_image : asset('assets/images/empty-box.png') }}"
                                    class="rounded-circle img-fluid" alt="Manager Inventory" width="80" height="80">
                            </div>
                            <div class="inventory-info text-center">
                                <span class="fw-bold mb-1" style="font-size: 0.8rem;">{{ $inventory->item_name }}</span>
                                <div class="text-secondary text-capitalize" style="font-size: 0.8rem;">Quantity:
                                    {{ $inventory->item_quantity }}</div>
    ...
    <script>
        ...
        document.getElementById('createInventoryBtn').addEventListener('click', function() {
            ...
            $.ajax({
                url: "{{ route('inventory-management.post') }}",
                type: 'POST',
                data: formData,
                ...
                success: function(response) {
                    ...
                    <div class="inventory-card shadow-sm" style="position: relative;">
                        ...
                        <div class="profile-picture mb-2">
                            <img src="${response.inventory.item_image ? '/storage/' + response.inventory.item_image : '{{ asset('assets/images/empty-box.png') }}'}"
                ...
                `;
                        ...
                        document.querySelector('.item-image img').src =
                            "{{ asset('assets/images/empty-box.png') }}";
