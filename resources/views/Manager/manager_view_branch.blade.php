...
@section('page-title', 'City Burgers POS | View Branch')

@section('content')
    <div class="container">
        <div class="return my-2 text-dark">
            <a href="{{ route('manager-dashboard.get') }}" class="text-dark"><i class="ph ph-arrow-circle-left me-1"></i></a>
            Back
        </div>
        ...
        <div class="branch-image-bg">
            <img class="rounded mb-3"
                src="{{ $branch->image ? '/storage/' . $branch->image : asset('assets/images/location.png') }}"
                alt="">
            <div class="branch-name">
                <div class="lead fw-bold text-light">{{ $branch->name }}</div>
                <div class="fw-bold text-light text-center">{{ $branch->address }}</div>
            </div>
        </div>
        ...
        <div class="assigned-cashier mt-4">
            <div class="d-flex justify-content-center gap-2 align-items-center mb-3">
                ...
                <a href="{{ route('manager-order-history.get', ['branch_id' => encrypt($branch->id)]) }}"
    ...
    <form action="{{ route('branch-management.assign', encrypt($branch->id)) }}" method="POST" class="modal fade"
                                ...
                                <div class="form mb-2">
                                    ...
                                    <select name="cashier_id" class="form-select" id="cashier">
                                        ...
                                        @foreach ($allCashiers as $cashier)
                                            <option value="{{ encrypt($cashier->id) }}">{{ $cashier->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
