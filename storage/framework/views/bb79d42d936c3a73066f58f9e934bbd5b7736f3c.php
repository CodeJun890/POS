...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const cancelButtonCross = document.querySelector('#offcanvasTop .btn-close');
            const cancelButton = document.querySelector('#offcanvasTop #cancelButton');

            // Function to reset prices
            function resetPrices() {
                // Find all price elements and reset their content
                const prices = document.querySelectorAll('.box-price span');

                // Define the default prices
                const defaultPrices = {
                    "Buy 1 Take 1 Sliders": "&#8369; 60",
                    "Manila Burger": "&#8369; 70",
                    "Berlin Burger Steak": "&#8369; 80",
                    "New York Burger": "&#8369; 110",
                    "French Fries(Solo)": "&#8369; 70",
                    "French Fries(Barkada)": "&#8369; 100",
                    "Mountain Dew": "&#8369; 30",
                    "Royal": "&#8369; 30",
                    "Sprite": "&#8369; 30",
                    "Coke": "&#8369; 30",
                ...
                // Loop through each price element and reset it
                prices.forEach(price => {
                    const burgerName = price.closest('.burger-box')?.dataset.burger || price.closest(
                            '.fries-box')?.dataset.fries || price.closest('.drink-box')?.dataset.drink ||
                        price.closest('.addons-box')?.dataset.addon;
                    if (burgerName && defaultPrices[burgerName]) {
                        price.innerHTML = defaultPrices[burgerName];
                    }
                });
                ...
                // Optional: Reset selected item details if needed
                document.getElementById('selected-item-title').innerHTML = '';
                document.getElementById('selected-item-img').src = '';
            }

            // Add click event listener to the cancel button (cross icon)
            if (cancelButtonCross) {
                cancelButtonCross.addEventListener('click', resetPrices);
            }

            // Add click event listener to the cancel button
            if (cancelButton) {
                cancelButton.addEventListener('click', resetPrices);
            }
        });
        ...
        $(document).ready(function() {
            ...
            if ($('.quantity__input').length && $('#offcanvasTop').length && $('.burger-box').length) {
                ...
                // Addons selection handling
                const addonsBoxes = $('.addons-box');
                setupItemSelection(addonsBoxes, 'selected-addons-title', 'selected-addons-img');
            ...
            if (
                ...
                saveButton.addEventListener('click', function() {
                    ...
                    function resetPrices() {
                        ...
                        // Loop through each price element and reset it
                        prices.forEach(price => {
                            const burgerName = price.closest('.burger-box')?.dataset.burger || price
                                .closest(
                                    '.fries-box')?.dataset.fries || price.closest('.drink-box')
                                ?.dataset.drink || price.closest('.addons-box')
                                ?.dataset.addon;
                            if (burgerName && defaultPrices[burgerName]) {
                                price.innerHTML = defaultPrices[burgerName];
                    ...
                    resetPrices();
                    // Determine if the selected item is a drink or an addon
                    const isDrink = document.querySelector(
                            '.burger-box.active, .fries-box.active, .drink-box.active, .addons-box.active')
                        ?.classList.contains('drink-box');

                    // Determine if the selected item is an addon
                    const isAddon = document.querySelector('.addons-box.active') !== null;

                    // Set selectedSauce based on whether the item is a drink or an addon
                    const selectedSauce = (isDrink || isAddon) ? null : (document.querySelector(
                    ...
                    orders.push({
                        ...
                        quantity: quantity,
                        sauce: selectedSauce, // This will be null if it's a drink or addons
                ...
                function renderOrders() {
                    ...
                    const customerNameContainer = `
                        <div class="mb-3 d-none" id="customerNameContainer">
                            <label for="customerName" class="form-label">Order No# (Optional)</label>
                            <input type="text" id="customerName" class="form-control" placeholder="Enter customer order number" />
                        </div>
                    `;
                    ...
                    orders.forEach((order, index) => {
                        ...
                        const drinks = ['Mountain Dew', 'Royal', 'Coke', 'Water (CvSU)', 'Water', 'Sprite'];
                ...
                // Item selection setup for burgers, fries, drinks, and addons
                document.querySelectorAll('.burger-box, .fries-box, .drink-box, .addons-box').forEach((box) => {
                    box.addEventListener('click', function() {
                        // Determine the type of item (burger, fries, drink, or addon)
                        const itemType = box.classList.contains('drink-box') ? 'drink' :
                            box.classList.contains('addons-box') ? 'addon' :
                            box.classList.contains('burger-box') ? 'burger' : 'fries';

                        // Update the selected item title and image
                        document.getElementById('selected-item-title').textContent = box.dataset
                            .burger || box.dataset.fries || box.dataset.drink || box.dataset.addon;
                        ...
                        // Show or hide sauce grid based on the selected item type
                        if (itemType === 'drink' || itemType === 'addon') {
                            document.querySelector('.sauce-grid').style.display = 'none';
                            document.querySelector('.offcanvas-top').style.height =
                                '55vh'; // Adjust height for drinks and addons
                        } else {
                            document.querySelector('.sauce-grid').style.display = 'flex';
                            document.querySelector('.offcanvas-top').style.height =
                                '75vh'; // Adjust height for burgers and fries
                        }
                    });
                });
            ...
            fetch('/orders/pending')
                ...
                .then(data => {
                    ...
                    if (!Array.isArray(data) || data.length === 0) { // Check if data is an array and if it's empty
                        ...
                        <span class="text-uppercase text-muted" style="font-size: 0.8rem;">
                            Order No#: <span class="text-success fw-bold">${group.customer_name}</span>
                        </span>`;
                        ...
                        <h2 class="accordion-header d-flex align-items-center justify-content-between" id="${headerId}" style="position:relative;">
                            <button class="accordion-button${index === 0 ? '' : ' collapsed'}" type="button" data-bs-toggle="collapse" data-bs-target="#${collapseId}" aria-expanded="${index === 0}" aria-controls="${collapseId}" style="color: #000; background-color: #ffffff; border: none;">
                                <span style="font-size: 0.8rem;">Payment Method: ${group.payment_method}</span>
                            </button>
                                    ...
                                    ${group.orders.map(order => `
                                                                                                                                                                                                                                            <div class="order-item d-flex align-items-center justify-content-between border-bottom py-2">
                                                                                                                                                                                                                                                <div class="order-details d-flex align-items-center">
                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                    <div class="ms-3">
                                                                                                                                                                                                                                                        <div class="fw-bold">${order.item} (${order.quantity}x)</div>
                                                                                                                                                                                                                                                        ${order.sauce && !['Mountain Dew', 'Royal', 'Coke', 'Water (CvSU)','Water', 'Sprite'].includes(order.item) ? `
